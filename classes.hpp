...
#ifndef COURSEWORK_CLASSES_HPP
#define COURSEWORK_CLASSES_HPP
	...
	void start(){
		try {
			if (machine->get_mode() == 'a' || machine->get_mode() == 'A') {
				...
				if (input == "y" || input == "Y"){
					...
					while (command != "n" && command != "N"){
						...
						if (input == "d" || input == "D"){
							...
							cin >> input;
							while (!is_int(input) || stoi(input) < 0 || stoi(input) > 5){
								cout << "" << termcolor::on_yellow << "Incorrect input, try again" << termcolor::reset << "\nNumber of container's sensor which should be disactivated:  ";
								cin >> input;
							}
				...
				system("cls");
				int i = 0;
				// Работаем пока не закончатся заготовки в каком-либо контейнере или процесс не будет остановлен нажатием клавиши
				while (!_kbhit()) {
